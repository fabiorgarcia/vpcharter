...
function ServicoBordo() {
  ...
  const [masterTitle, setMasterTitle] = useState ('Serviço de Bordo');
  ...
  function validaForm(e) {
    ...
    setTimeout(function() {
      if (!newData) {
        var query = "UPDATE `vpcharter_servico_bordo` SET `servico_bordo` = '"+servicoBordo.trim()+"' WHERE `vpcharter_servico_bordo`.`id` = '"+idData+"';";
        var fData = new FormData();
        fData.append('query', query);
        axios.post(endpoint+'query.php', fData)
        .then(response=> {
        ...
        var query = "SELECT * FROM `vpcharter_servico_bordo` WHERE `id` = '"+idData+"' ";
        var fData = new FormData();
        fData.append('query', query);
        axios.post(endpoint+'query.php', fData)
        .then(response=> {
          if (response.data.length > 0) {
            ...
            var query = "INSERT INTO `vpcharter_servico_bordo` (`id`, `servico_bordo`) VALUES (NULL, '"+servicoBordo+"') ";
            var fData = new FormData();
            fData.append('query', query);
            axios.post(endpoint+'query.php', fData)
            .then(response=> {
  ...
  function listSearch() {
    ...
    setTypeAlert('')
    setIdData('')
    var query = "SELECT * FROM `vpcharter_servico_bordo` WHERE `servico_bordo` LIKE '%"+buscaFiltro+"%' ORDER BY `servico_bordo` ";
    var fData = new FormData();
    fData.append('query', query);
    axios.post(endpoint+'query.php', fData)
    .then(response=> {
                  ...
                  <form onSubmit={(e)=> validaForm(e)}>
                    <div className='row'>
                      ...
                      <div className='col-9'>
                        ...
                        <input type='text' value={servicoBordo} onChange={(e) => setServicoBordo(e.target.value)} required />
                      </div>
                ...
                <div className='table_list'> 
                  ...
                  <table className='tblDefault'>
                    ...
                    {listData.map((data, index) => (
                      <tr key={index} onClick={() => editaData(data.id, data.servico_bordo)} className={idData == data.id?'trSelect':''}>
                        <td>{data.id}</td>
